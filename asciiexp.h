...
#define CFGFILENAME		_T("dy_Export.CFG")
...
public:
	...
	void ExportSkinName(INode* pNode, Modifier* pMod, int NumVert);
	void ComputeTangentVector(INode* node, Mesh* mesh, bool bNegScale, TimeValue t);
	...
	inline void AddToFloat3(XMFLOAT3& dest, const XMFLOAT3& src)
	{
		XMVECTOR v1 = XMLoadFloat3(&dest);
		XMVECTOR v2 = XMLoadFloat3(&src);
		v1 = XMVectorAdd(v1, v2);
		XMStoreFloat3(&dest, v1);
	}
...
private:
	...
	//Ãß°¡
	std::vector<Mtl*> m_vMtl;
	std::unordered_map<std::wstring, int> m_BoneMap;
	int m_BoneCounter = 0;
	std::unordered_map<UINT, XMFLOAT3> m_mTangent;
	std::unordered_map<UINT, XMFLOAT3> m_mBinormal;

	std::vector<Mtl*> SortMtrl();
	std::vector<Mtl*> DuplicationNameDelete(std::vector<Mtl*>& vMtl);
};
